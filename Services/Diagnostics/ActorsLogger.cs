...
namespace Microsoft.Azure.IoTSolutions.DeviceSimulation.Services.Diagnostics
{
    ...
    public class ActorsLogger : IActorsLogger
    {
        ...
        public void AddDeviceToStoreScheduled(long time)
        {
            if (!this.enabled) return;

            var msg = DateTimeOffset.FromUnixTimeMilliseconds(time).ToString(DATE_FORMAT);
            this.Log("Add device to store scheduled at: " + msg);
            this.LogRegistry("Add device to store scheduled at: " + msg);
        }

        public void AddingDeviceToStore()
        {
            if (!this.enabled) return;

            this.Log("Adding device to store");
            this.LogRegistry("Adding Device to store");
        }

        public void AddedDeviceToStore()
        {
            if (!this.enabled) return;

            this.Log("Device added to store");
            this.LogRegistry("Added to store");
        }

        public void AddDeviceToStoreFailed()
        {
            if (!this.enabled) return;

            this.Log("Device add to store FAILED");
            this.LogRegistry("Add to store FAILED");
        }

        public void DeleteDeviceFromStoreScheduled(long time)
        {
            if (!this.enabled) return;

            var msg = DateTimeOffset.FromUnixTimeMilliseconds(time).ToString(DATE_FORMAT);
            this.Log("Delete device from store scheduled at: " + msg);
            this.LogRegistry("Delete device from store scheduled at: " + msg);
        }

        public void DeletingDeviceFromStore()
        {
            if (!this.enabled) return;

            this.Log("Deleting device from store");
            this.LogRegistry("Deleting from store");
        }

        public void DeletedDeviceFromStore()
        {
            if (!this.enabled) return;

            this.Log("Device deleted from store");
            this.LogRegistry("Deleted from Store");
        }

        public void DeleteDeviceFromStoreFailed()
        {
            if (!this.enabled) return;

            this.Log("Device delete from store FAILED");
            this.LogRegistry("Delete from store FAILED");
        }
